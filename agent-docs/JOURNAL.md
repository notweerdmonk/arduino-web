...
**Tooling**: Source review, ruff with bandit rules, dependency audit.
**Findings**: 24 total — 5 Critical, 8 High, 7 Medium, 4 Low.

**Critical findings**:
- No authentication on any endpoint (CWE-306)
- Hardcoded `"dev-secret"` fallback for Flask session key (CWE-798)
- Cleartext gRPC with no TLS (CWE-319)
- No CSRF protection on any state-changing endpoint (CWE-352)
- World-writable UDS socket `0o666` (CWE-276)

**Multi-user deployment risk**: The codebase was designed for single-user local use. All 5 critical issues are trivially exploitable on a LAN, making it unsuitable for multi-user or internet-facing deployment without major authentication, authorization, CSRF, rate-limiting, and encryption work.

**Internet-facing deployment risk**: Every finding becomes directly exploitable from anywhere in the world. An attacker can delete medicine schedules, upload malicious firmware to connected Arduino boards via compiled sketches, intercept gRPC traffic (MITM), forge session cookies, and perform CSRF-driven attacks against browser users. The blast radius includes all data, all connected hardware, and system availability.

**Documentation**: Full findings recorded in `AUDIT_FINDINGS.md`. Plan added to `PLAN.md` (Phase 110: Authentication, Authorization, CSRF, Rate Limiting).
