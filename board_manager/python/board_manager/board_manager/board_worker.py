"""board_manager/python/board_manager/board_manager/board_worker.py
...
def _handle_message(msg: dict, client: Any, sock: socket.socket) -> None:
    ...
    if method == "init":
        ...
        try:
            ...
            for out, err, done, percent in client.compile_stream(
                ...
                if done:
                    ...
                    last_pct = percent
            logger.info(
                "compile: done success=%s output_len=%d",
                success,
                sum(len(line) for line in out_lines),
            )
            sock.sendall(
                encode_and_frame(
                    _make_result(
        ...
        try:
            ...
            for out, err, done in client.upload_stream(
                ...
                if done:
                    success = True
            logger.info(
                "upload: done success=%s output_len=%d",
                success,
                sum(len(line) for line in out_lines),
            )
            if success:
        ...
        try:
            ...
            verbose = params.get("verbose", False)
            logger.info(
                "compile_and_upload: starting sketch=%s fqbn=%s port=%s",
                sketch_path,
                fqbn,
                port,
            )
