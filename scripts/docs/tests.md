...
| `test_ci.sh` | bash | 49 | CI pipeline: skip flags, exit codes, nox invocation, lint phase, lock file handling |

**Total:** 189 tests (128 pytest + 61 bash)
...
```bash
nox -s scripts_tests    # runs all 189 tests
```
...
pipenv run bash tests/test_ci.sh                      # 49 bash tests
```
